        ...
        for samples in tqdm(test_data_loader):
            ...
            with open(args.csv_output_path, 'a', newline='') as file:
                ...
                for info in zip(*all_predictions, caption_info):
                    row = [info[-1][0], info[-1][1], info[-1][2], info[-1][3]] + list(info[:-1])
                    writer.writerow(row)

if __name__ == "__main__":
    window_size = 15
    parser = argparse.ArgumentParser(description="Train a model with FRANZ dataset.")
    parser.add_argument("--feature_root", type=str, default="./finetune/clip_embeddings/test")
    parser.add_argument("--ann_root", type=str, default="./finetune/videos/test")
    ...
    parser.add_argument("--csv_output_path", type=str, default=f"./inference_result/goal_ws/predict_clip_window_{window_size}.csv", help="the path to the output predictions")
